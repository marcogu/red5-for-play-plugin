...
public class Input extends BaseInput implements org.red5.io.object.Input {
	...
	public String readString(Type target) {
		...
		switch (currentDataType) {
			...
			case AMF.TYPE_STRING:
				...
				break;
			default:
				Logger.debug("Unknown AMF type: {}", currentDataType);
		}
		int limit = buf.limit();
	...
	public static String getString(IoBuffer buf) {
		...
		Logger.debug("String buf - position: {} limit: {}", pos, (pos + len));
	...
	public Object readArray(Deserializer deserializer, Type target) {
		Logger.debug("readArray - deserializer: {} target: {}", deserializer, target);
		Object result = null;
	...
	public Object readMap(Deserializer deserializer, Type target) {
		...
		Logger.debug("Read start mixed array: {}", maxNumber);
		Object result;
		...
		while (hasMoreProperties()) {
			...
			try {
				...
				Logger.debug("key {} is causing non normal array", key);
				normal_array = false;
			}
		...
		if (mixedResult.size() <= maxNumber+1 && normal_array) {
			...
			Logger.debug("mixed array is a regular array");
...
//			instance = clazz.newInstance();
			instance = newInstanceFromPlayFramework(className);
		} catch (Exception ex) {
	 ...
	 * @return
	 */
	protected Object newInstanceFromPlayFramework(String className){
		@SuppressWarnings("rawtypes")
		Class clazz = null;
		try {
			ApplicationClass classWrapper = Play.classes.getApplicationClass(className);
			if(classWrapper==null || classWrapper.javaClass==null){
				clazz = Play.classloader.loadClass(className);
			}else{
				clazz = classWrapper.javaClass;
			}
	...
	protected Object readBean(Deserializer deserializer, Object bean) {
		...
		while (hasMoreProperties()) {
			...
			Logger.debug("val: {}", property);
			//Logger.debug("val: "+property.getClass().getName());
			try {
				if (property != null) {
					try {
					...
					Logger.debug("Skipping null property: {}", name);
				}
			} catch (Exception ex) {
				Logger.error("Error mapping property: {} ({})", name, property);
			}
			if (hasMoreProperties()) {
				skipPropertySeparator();
			}
	...
	public Object readObject(Deserializer deserializer, Type target) {
		...
		Logger.debug("readObject: {}", className);
		Object result = null;
		if (className != null) {
			Logger.debug("read class object");
			Object instance;
	...
	public boolean hasMoreProperties() {
		...
		Logger.debug("End of object: ? {}", isEndOfObject);
	...
	public Document readXML(Type target) {
		...
		try {
			...
			Logger.error("IOException converting xml to dom", ioex);
		}
		storeReference(doc);
		return doc;
	}
