...
public class Input extends org.red5.io.amf.Input implements org.red5.io.object.Input {
		...
		public void resolveProperties(Object result) {
			...
			for (PendingProperty prop: properties) {
				try {
					...
					Logger.error("Error mapping property: {} ({})", prop.name, result);
				}
			}
			properties.clear();
		}
	...
	public byte readDataType() {
		...
		Logger.debug("Current data type: {}", currentDataType);
		...
		Logger.debug("Current data type (after amf checks): {}", currentDataType);
		...
		switch (currentDataType) {
			...
			default:
				Logger.info("Unknown datatype: {}", currentDataType);
				// End of object, and anything else lets just skip
				coreType = DataTypes.CORE_SKIP;
				break;
		}
		Logger.debug("Core type: {}", coreType);
		return coreType;
	}
	...
	public String readString(Type target) {
		...
		Logger.debug("readString - length: {}", len);
		if (len == 1) {
			// Empty string
			return "";
		}
		if ((len & 1) == 0) {
			...
			if (stringReferences.isEmpty()) {
				Logger.debug("String reference list is empty");
			}
			// Reference
			return stringReferences.get(len >> 1);
		}
		len >>= 1;
		Logger.debug("readString - new length: {}", len);
		int limit = buf.limit();
		Logger.debug("readString - limit: {}", limit);
	...
	public Object readObject(Deserializer deserializer, Type target) {
		...
		case AMF3.TYPE_OBJECT_EXTERNALIZABLE:
			...
			Logger.debug("Externalizable class: {}", className);
					...
					for (Map.Entry<String, Object> entry: properties.entrySet()) {
						...
						try {
							if (value != null) { 
								...
								Logger.debug("Skipping null property: {}", key);
							}
						} catch (Exception e) {
							Logger.error("Error mapping property: {} ({})", key, value);
						}
					}
				} // else fall through
			}
