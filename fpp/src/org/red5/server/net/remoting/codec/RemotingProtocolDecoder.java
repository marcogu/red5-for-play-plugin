...
public class RemotingProtocolDecoder implements SimpleProtocolDecoder {
    ...
    protected Map<String, Object> readHeaders(IoBuffer in) {
		...
		for (int i = 0; i < count; i++) {
			...
			Logger.debug("Header: {} Required: {} Size: {} Value: {}", new Object[]{name, required, size, value});
	...
	protected List<RemotingCall> decodeCalls(IoBuffer in) {
		...
		for (int i = 0; i < count; i++) {
			...
			Logger.debug("callback: {}", clientCallback);
			...
			if (type == AMF.TYPE_ARRAY) {
				...
				Logger.debug("Got null amf type");
			...
			Logger.debug("Service: {} Method: {}", serviceName, serviceMethod);
